...
Template.stats.events({
	'click button.downloadData': function() {
...
Template.playerGameRow.notFirst = function() {
	return currentPlayers[0]._id != this._id ? true : false;
};

Template.playerGameRow.notLast = function() {
	return currentPlayers[currentPlayers.length-1]._id != this._id ? true : false;
};
...
Template.playerGameRow.events({
	...
	'click button.moveUp': function() {
		for (i in currentPlayers) {
			var ii = parseInt(i);
			if (currentPlayers[i]._id == this._id) {
				var player = currentPlayers[i];
				currentPlayers[i] = currentPlayers[ii - 1];
				currentPlayers[ii - 1] = player;
				currentPlayersDepend.changed();
				return;
			}
		}
	},
	'click button.moveDown': function() {
		for (i in currentPlayers) {
			var ii = parseInt(i);
			if (currentPlayers[i]._id == this._id) {
				var player = currentPlayers[i];
